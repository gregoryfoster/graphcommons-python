...
class Graph(Entity):
    def __init__(self, *args, **kwargs):
        ...
        self.edges = list(map(Edge, self.get('edges', [])))
        self.nodes = list(map(Node, self.get('nodes', [])))
        self.node_types = list(map(NodeType, self.get('nodeTypes', [])))
        self.edge_types = list(map(EdgeType, self.get('edgeTypes', [])))
